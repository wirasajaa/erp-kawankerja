<x-auth-app>
    <x-card title="Add new family">
        <form action="{{ route('family.update', ['family' => $family->id]) }}" method="post">
            @csrf
            @method('put')
            <div class="row mb-md-3">
                <div class="col-md-6 mb-3 mb-md-0">
                    ...
                    <x-text-input name="identity_number" type="text" id="inputIdentity"
                        value="{{ old('identity_number', $family->identity_number) }}" />
                </div>
                <div class="col-md-6 mb-3 mb-md-0">
                    <x-label-input for="inputName">Name</x-label-input>
                    <x-text-input name="name" type="text" id="inputName"
                ...
                <div class="col-md-6 mb-3 mb-md-0">
                    ...
                    <x-select-input name="relationship" id="inputRelation">
                        @foreach ($relations as $item)
                            <option
                                value="{{ $item->value }}"{{ $item->value == old('relationship', $family->relationship) ? 'selected' : '' }}>
                                {{ $item->name }}
                            </option>
                        @endforeach
                    </x-select-input>
                    ...
                    <div class="d-flex gap-2">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="gender" id="Male"
                                value="MALE"{{ old('gender', $family->gender) == 'MALE' ? 'checked' : '' }}>
                            <label class="form-check-label" for="Male">MALE</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="gender" id="Female"
                                value="FEMALE"{{ old('gender', $family->gender) == 'FEMALE' ? 'checked' : '' }}>
