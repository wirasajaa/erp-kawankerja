    ...
    <x-card title="Add New Role">
        <form action="{{ route('roles.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <x-text-input name="name" id="inputName" placeholder="Fullstack Developer" value="{{ old('name') }}"
                    type="text" />
            </div>
                    ...
                    <div class="col-6 col-md-3">
                        <div class="form-check">
                            <input class="form-check-input{{ $errors->has('permissions') ? ' is-invalid' : '' }}"
                                type="checkbox"
                                id="{{ 'input' . $key }}"{{ collect(old('permissions'))->contains($permission['name']) ? 'checked' : '' }}
